...
namespace Diadoc.Api
{
	public partial class DiadocHttpApi
	{
		...
		[NotNull]
		public DocumentList GetDocumentsV4([NotNull] string authToken, string boxId, GetDocumentsV4Request request)
		{
			var qsb = new PathAndQueryBuilder("/V4/GetDocuments");
			qsb.AddParameter("boxId", boxId);
			return PerformHttpRequest<DocumentList>(authToken, "POST", qsb.BuildPathAndQuery(), Serialize(request));
		}
			...
			params DocumentHistoryEmbed[] embeds)
		{
			...
			return PerformHttpRequest<DocumentHistory>(authToken, "GET", qsb.BuildPathAndQuery());
		}

		[NotNull]
		public DocumentActionList GetDocumentActions([NotNull] string authToken, [NotNull] string boxId, [NotNull] string messageId, [NotNull] string entityId)
		{
			var qsb = new PathAndQueryBuilder("/GetDocumentActions");
			qsb.AddParameter("boxId", boxId);
			qsb.AddParameter("messageId", messageId);
			qsb.AddParameter("entityId", entityId);
			return PerformHttpRequest<DocumentActionList>(authToken, "GET", qsb.BuildPathAndQuery());
		}
		...
		public DocumentWorkflowSettingsListV3 GetWorkflowsSettingsV3([NotNull] string authToken, string boxId)
		{
			var qsb = new PathAndQueryBuilder("/V3/GetWorkflowsSettings");
			qsb.AddParameter("boxId", boxId);
			return PerformHttpRequest<DocumentWorkflowSettingsListV3>(authToken, "GET", qsb.BuildPathAndQuery());
		}
