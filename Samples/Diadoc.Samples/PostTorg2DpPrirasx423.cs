...
namespace Diadoc.Samples
{
	...
	public class PostTorg2DpPrirasx423
	{
		...
		public static void Sample()
		{
			...
			var function = "NoAdditionalInfo"; // функция, представляющая акт об установленном расхождении
			var version = "torg2_05_01_01"; // версия, отвечающая за то, что документ сформирован в формате приказа №423
		...
		private static Torg2SenderTitle BuildUserDataContract()
		{
			...
			var torg2SellerTitleInfo = new Torg2SenderTitle
			{
				...
				Signers = GetSigners(),
				DocumentAdditionalInfos = new[]
				{
					Torg2SenderTitleDocumentAdditionalInfo.Item1
				}
			};
				...
				AcceptanceResults = new[]
				{
					new AcceptanceResult
					{
						...
						AccordingToSellerDocument = new AcceptedProductData
						{
							Quantity = 100,
							Price = 144,
							TaxRate = TaxRate_Torg2.EighteenPercent
						}
					}
				},
				EventSummary = "Продажа"
			};
		}
			...
			return new CircumstancesAcceptanceInfo
			{
				Seller = new ExtendedOrganizationInfo_Torg2
				{
					Item = new ExtendedOrganizationDetails_Torg2
					{
						Inn = "7750370238",
						Kpp = "770100101",
						OrgType = (OrganizationType) Api.DataXml.OrganizationType.LegalEntity,
