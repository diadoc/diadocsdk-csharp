...
{
	[TestFixture]
	internal static class PostUniversalCorrectionDocument29
	{
		[Test]
		public static void Sample()
		{
			Console.WriteLine("Пример отправки универсального корректировочного документа (УКД) в формате приказа №29");
			Console.WriteLine("===================================================================================");
			...
			var function = "КСЧФДИС"; // функция, представляющая УКД как корректировочный счёт-фактуру
			var version = "ucd29_05_02_01"; // версия, отвечающая за то, что документ сформирован в формате приказа №29
			...
			var userDataContract = Ucd29Helper.BuildUserDataContract(messageToPost.FromBoxId, certificateRawData, powerOfAttorneyType);
			...
			var documentAttachment = new DocumentAttachment
			{
				...
				Для наших целей (УКД с функцией КСЧФДИС в формате приказа №29) мы уже подобрали нужную комбинацию выше:
				*/
				TypeNamedId = typeNamedId,
				Function = function,
				Version = version,

				// Теперь передадим сам файл УКД и сформированную к нему подпись и МЧД:
				SignedContent = Ucd29Helper.BuildSignerContent(content, signature, powerOfAttorneyType),
			...
			Console.WriteLine("Отправляем пакет из одного УКД с функцией КСЧФДИС...");
