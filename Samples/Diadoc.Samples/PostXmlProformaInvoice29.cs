...
using PositionPositionSource = Diadoc.Api.DataXml.ON_CHETOP_UserContract_05_01_01.PositionPositionSource;
...
namespace Diadoc.Samples
{
	...
	internal static class PostXmlProformaInvoice29
	{
		...
		public static void Sample()
		{
			...
			var typeNamedId = "XmlProformaInvoice"; // строковый идентификатор типа Счет
			var function = "default"; // функция
			var version = "proformainvoice29_05_01_01"; // версия, отвечающая за то, что документ сформирован в формате приказа №29
			...
			var signature = crypt.Sign(content, certificateRawData); // здесь лежит бинарное представление подписи к Счету
			...
			var documentAttachment = new DocumentAttachment
			{
				...
				Для наших целей (Счет в формате приказа №29) мы уже подобрали нужную комбинацию выше:
				*/
		...
		private static XmlProformaInvoice BuildUserDataContract(string senderBoxId, byte[] certificateRawData, PowerOfAttorneyType powerOfAttorneyType)
		{
			...
			{
				Function = XmlProformaInvoiceFunction.Item0,
